...
function doMain() {
  ...
  if (rtType == 1) {
    ...
    function chgScale(myScale) {
      var obStyleNames01, obStyle01Spl, obStyle01, obStyle01, i, iL, pgItms;
      ...
      for (i = 0, iL = pgItms.length; i < iL; i++) {
        if (pgItms[i].appliedObjectStyle.name == obStyle01) {
          //タテ倍率に入力値を反映
          pgItms[i].graphics[0].verticalScale = myScale / 1;
          //ヨコ倍率に入力値を反映
          pgItms[i].graphics[0].horizontalScale = myScale / 1;
          //フレームを内容に合わせる
          pgItms[i].fit(FitOptions.FRAME_TO_CONTENT);
        }
      }
    }
